...
const LocalPanel: React.FC = () => {
  const settingsSets = useStore(state => state.local.settingsSets);
  const groups = useStore(state => state.local.groups);
  const visualOrder = useStore(state => state.local.visualOrder);
  ...
  const handleKeyDown = React.useCallback((e: KeyboardEvent) => {
    ...
    if (e.key === 'Delete' && currentSelectedIds.length > 0) {
      ...
      currentSelectedIds.forEach(id => {
        ...
        deleteItem('local', id, isGroup ? 'group' : 'settingsSet');
      });
      setSelectedIds([]);
    ...
    if (e.key === 'ArrowUp') {
      ...
      if (settingsSet && settingsSet.groupId) {
        ...
        if (currentIndex > 0) {
          ...
          reorderWithinGroup('local', settingsSet.groupId, newChildren);
        } else {
          // Exit group and move to root level above the group
          const groupIndex = visualOrder.indexOf(settingsSet.groupId);
          moveItemToRoot('local', currentLastSelected, groupIndex);
        }
      } else {
        ...
        if (currentIndex > 0) {
          ...
          if (prevIsGroup && settingsSet) {
            ...
            moveItemToGroup('local', currentLastSelected, prevItemId, prevIsGroup.children.length);
          } else {
            // Normal root-level swap
            const newOrder = [...visualOrder];
            [newOrder[currentIndex - 1], newOrder[currentIndex]] = [newOrder[currentIndex], newOrder[currentIndex - 1]];
            reorderItems('local', newOrder);
          }
        }
      }
      ...
      if (settingsSet && settingsSet.groupId) {
        ...
        if (currentIndex < parentGroup.children.length - 1) {
          ...
          const groupIndex = visualOrder.indexOf(settingsSet.groupId);
          moveItemToRoot('local', currentLastSelected, groupIndex + 1);
        }
      } else {
        // Item is at root level (either settings set or group)
        const currentIndex = visualOrder.indexOf(currentLastSelected);
        if (currentIndex < visualOrder.length - 1) {
          ...
          if (nextIsGroup && settingsSet) {
            ...
            moveItemToGroup('local', currentLastSelected, nextItemId, 0);
          } else {
            // Normal root-level swap
            const newOrder = [...visualOrder];
  ...
  const moveItemUp = (itemId: string) => {
    ...
    if ('groupId' in item && item.groupId) {
      ...
      if (currentIndex > 0) {
        ...
        [newChildren[currentIndex - 1], newChildren[currentIndex]] = [newChildren[currentIndex], newChildren[currentIndex - 1]];
        reorderWithinGroup('local', item.groupId, newChildren);
      } else {
        // Exit group and move to root level above the group
        const groupIndex = visualOrder.indexOf(item.groupId);
        moveItemToRoot('local', itemId, groupIndex);
      }
    } else {
  ...
  const moveItemDown = (itemId: string) => {
    ...
    if ('groupId' in item && item.groupId) {
      ...
      if (currentIndex < group.children.length - 1) {
        ...
        // Exit group and move to root level below the group
        const groupIndex = visualOrder.indexOf(item.groupId);
        moveItemToRoot('local', itemId, groupIndex + 1);
      }
    } else {
      // Item is at root level
      const currentIndex = visualOrder.indexOf(itemId);
      if (currentIndex < visualOrder.length - 1) {
        ...
        if (nextGroup && 'groupId' in item === false) {
          // Next item is a group and current item is a settings set - enter the group
          moveItemToGroup('local', itemId, nextItemId, 0);
        } else {
          // Normal root-level swap
          const newOrder = [...visualOrder];
          [newOrder[currentIndex], newOrder[currentIndex + 1]] = [newOrder[currentIndex + 1], newOrder[currentIndex]];
  ...
  const openContextMenu = (e: React.MouseEvent, id: string, type: 'settingsSet' | 'group') => {
    ...
    if (type === 'settingsSet' && isMulti) {
      actions = [
        { 
          label: 'Group', 
          onClick: () => { groupItems('local', currentSelection); setSelectedIds([]); }
        },
        { 
          label: 'Delete', 
          onClick: () => {
            currentSelection.forEach(selectedId => deleteItem('local', selectedId, 'settingsSet'));
            setSelectedIds([]);
          }
        },
      ];
      ...
      actions = [
        ...
        { 
          label: 'Delete', 
          onClick: () => {
            deleteItem('local', id, 'settingsSet');
            setSelectedIds([]);
          }
        },
      ];
  ...
  return (
    <div style={{ padding: '1rem', minHeight: '500px' }} onClick={handleCanvasClick}>
      <div style={{ display: 'flex', alignItems: 'center', marginBottom: 12 }}>
        <h2 style={{ flex: 1, fontSize: '1.2rem', margin: 0 }}>Local Panel</h2>
        <button
        ...
        <button
          ...
          onClick={() => addSettingsSet('local', { name: 'New Settings Set' })}
        >
          + Add Settings Set
        </button>
      </div>
          ...
          return (
            ...
            >
              <Group
                ...
                onRename={(newName) => renameGroup('local', item.id, newName)}
                onChildRename={(childId, newName) => renameSettingsSet('local', childId, newName)}
              />
            </div>
          );
        } else {
          return (
            ...
            >
              <SettingsSetComponent
                ...
                onRename={(newName) => renameSettingsSet('local', item.id, newName)}
              />
            </div>
          );
        }
